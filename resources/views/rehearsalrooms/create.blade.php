@extends('layouts.app', ['page' => __('rehearsalrooms.add'), 'pageSlug' => 'rehearsalrooms'])
@section('content')
<div class="bm-card">
    <div class="bm-card-header">
        <h2 class="bm-card-title">{{ __('rehearsalrooms.add') }}</h2>
    </div>
    <div class="bm-card-body">
        <form action="{{ route('rehearsalrooms.store') }}" method="post" enctype="multipart/form-data">
            ...
            <div class="bm-form-group">
                ...
                <input type="text" id="city" name="city" value="{{ old('city') }}" class="bm-input" placeholder="{{ __('rehearsalrooms.city_placeholder') }}" required>
                @error('city') <span class="bm-error">{{ $message }}</span> @enderror
            </div>
            <div class="bm-form-group">
                <label for="country" class="bm-label">{{ __('rehearsalrooms.country') }}</label>
                <input type="text" id="country" name="country" value="{{ old('country') }}" class="bm-input" placeholder="{{ __('rehearsalrooms.country_placeholder') }}" required>
            ...
            <div class="bm-form-group">
                ...
                <textarea id="description" name="description" class="bm-input" rows="4" placeholder="{{ __('rehearsalrooms.description_placeholder') }}">{{ old('description') }}</textarea>
